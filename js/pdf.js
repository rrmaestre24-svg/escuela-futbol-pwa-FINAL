...
function generateInvoicePDF(paymentId, autoDownload = true) {
  ...
  // Logo (convertir base64 a imagen)
  if (settings.logo) {
    try {
  ...
  doc.text(settings.name, 50, 25);
  
  doc.setFontSize(10);
  doc.setTextColor(...textColor);
  doc.setFont(undefined, 'normal');
  doc.text(settings.email || '', 50, 32);
  doc.text(settings.phone || '', 50, 37);
  doc.text(settings.address || '', 50, 42);
  ...
  doc.setFontSize(12);
  doc.setTextColor(...textColor);
  doc.text(payment.invoiceNumber, 150, 35);
  ...
  doc.text(`Categoría: ${player.category}`, 15, 75);
  doc.text(`Teléfono: ${player.phone}`, 15, 82);
  if (player.email) {
  ...
  doc.text('CANTIDAD', 100, tableTop + 7);
  ...
  doc.line(15, rowTop + 5, 195, rowTop + 5);
  ...
  if (payment.method) {
    ...
    doc.setFontSize(10);
    doc.setFont(undefined, 'normal');
    doc.text(`Método de pago: ${payment.method}`, 15, totalsTop + 10);
  }
  ...
  const footerText = 'Gracias por tu preferencia';
  doc.text(footerText, 105, 270, { align: 'center' });
  
  doc.setFontSize(8);
  doc.text(settings.name, 105, 275, { align: 'center' });
  doc.text(`Generado el ${formatDate(getCurrentDate())}`, 105, 280, { align: 'center' });
  
  // Guardar o retornar
  if (autoDownload) {
    doc.save(`Factura-${payment.invoiceNumber}.pdf`);
...
// Generar notificación de pago PDF
function generatePaymentNotificationPDF(paymentId) {
  ...
  if (daysDiff > 0 && daysDiff <= 10) {
    ...
    title = '⚠️ PAGO EN PERÍODO DE GRACIA';
    statusColor = yellowColor;
  } else {
    title = '🚨 PAGO VENCIDO';
  ...
  doc.text(`Categoría: ${player.category}`, 15, 92);
  ...
  if (daysDiff < 0) {
    ...
    doc.setFont(undefined, 'bold');
    doc.text(`Días de retraso: ${Math.abs(daysDiff)} días`, 15, 141);
  } else if (daysDiff <= 10) {
    doc.setTextColor(...yellowColor);
    doc.setFont(undefined, 'bold');
    doc.text(`Días para vencimiento: ${daysDiff} días`, 15, 141);
  }
  ...
  let message = '';
  if (daysDiff < -40) {
    message = 'Su pago se encuentra VENCIDO. Le solicitamos regularizar su situación a la brevedad posible.';
  } else if (daysDiff <= 0) {
    message = 'Su pago se encuentra en período de gracia. Le agradecemos ponerse al día lo antes posible.';
  } else {
    message = 'Le recordamos que tiene un pago próximo a vencer. Por favor, realice el pago antes de la fecha límite.';
  }
  ...
  doc.setFont(undefined, 'bold');
  doc.text('Para más información, contáctenos:', 15, 190);
  doc.setFont(undefined, 'normal');
  doc.text(`Teléfono: ${settings.phone}`, 15, 197);
  doc.text(`Email: ${settings.email}`, 15, 204);
...
// Generar estado de cuenta por jugador PDF
function generatePlayerAccountStatementPDF(playerId) {
  ...
  doc.text(`Categoría: ${player.category}`, 15, 72);
  ...
  payments.forEach((p, index) => {
    ...
    doc.text(p.concept.substring(0, 25), 50, yPos + 5);
...
// Generar reporte contable completo PDF
function generateFullAccountingReportPDF() {
  ...
  doc.setFontSize(32);
  doc.setFont(undefined, 'bold');
  doc.text(settings.name, 105, 150, { align: 'center' });
